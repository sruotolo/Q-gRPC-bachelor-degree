...
# --- BRENCIS CONSTANTS (2001) ---
BRENCIS_REAL_IP = os.getenv("BRENCIS_REAL_IP")
BRENCIS_REAL_PORT = int(os.getenv("BRENCIS_REAL_PORT"))
BRENCIS_CERT_PATH = os.getenv("BRENCIS_CERT_PATH")
BRENCIS_KEY_PATH = os.getenv("BRENCIS_KEY_PATH")
BRENCIS_CA_PATH = os.getenv("BRENCIS_CA_PATH")
...
def split_and_hash(key_id, key):
    ...
    hash_key_L = sha256_hash(key_L)

    return {
        "key_ID": key_id,
        "part_of_key": "RIGHT",
        "key_half_b64": base64.b64encode(key_R).decode('utf-8'),
        "other_half_hash_b64": base64.b64encode(hash_key_L).decode('utf-8')
    }
...
def forward_get_key(slave_sae_id):
    ...
    url = f"https://{BRENCIS_REAL_IP}:{BRENCIS_REAL_PORT}/api/v1/keys/{slave_sae_id}/enc_keys"

    try:
        response = requests.post(
            url,
            cert=(BRENCIS_CERT_PATH, BRENCIS_KEY_PATH),
            verify=BRENCIS_CA_PATH,
...
def forward_get_key_with_id(master_sae_id):
    ...
    url = f"https://{BRENCIS_REAL_IP}:{BRENCIS_REAL_PORT}/api/v1/keys/{master_sae_id}/dec_keys"
...
if __name__ == '__main__':
    print("BRENCIS proxy is ready on 0.0.0.0:5002")
