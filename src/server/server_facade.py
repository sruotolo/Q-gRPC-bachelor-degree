...
class ServerFacade(BaseFacade, QKDOrchestratorServiceServicer):
    ...
    It has all the encryption/decryption logic and the gRPC logic using a dictionary of active sessions that is used
    ...
    def NegotiateNetworkTopology(self, request: NetworkTopologyRequest, context: grpc.ServicerContext) -> NetworkTopologyResponse:
        ...
        client_adapter, partner_adapter = self.setup_network_connection(client_kdc_name, partner_kdc_name)
    ...
    def ButterflySynchronization(self, request: ButterflySyncRequest, context: grpc.ServicerContext) -> ButterflySyncResponse:
        ...
        try:
            for adapter in [client_adapter, partner_adapter]:
                ...
                hashes[part_of_key] = other_hash_bytes
        ...
        try:
            ...
            if (crypto_utils.sha256_hash(keys["LEFT"]) != hashes["RIGHT"] or
                    crypto_utils.sha256_hash(keys["RIGHT"]) != hashes["LEFT"]):
